...
//    }
    ...
    public void ChangeFileFormat(String outputFile) throws FileNotFoundException, UnsupportedEncodingException {
        Map<Integer, TreeNode> wholeTree = GetTreeMap();
        TreeNode root = wholeTree.get(FileTransfer.STARTINDEX);
        DFS(root, null, 0);
        ...
        INDEX = 0;
        for (TreeNode node : wholeTree.values()) {
            node.resetSibling();
        }
    ...
    private void BuildData(List<List<Float>> points, List<List<Integer>> connectivity, List<Double> distance, TreeNode root) {
        ...
        while (temp != null) {
            if (temp.getChildren().size() != 1 || temp.getParentNode() == null) {
                temp.setIndex(INDEX++);
                points.add(temp.getVal().getPosition());
                if (temp.getParentNode() != null) {
                    List<Integer> connect = new ArrayList<Integer>();
                    connect.add(temp.getParentNode().getIndex());
                    connect.add(temp.getIndex());
                    connectivity.add(connect);
                    distance.add(temp.getDistance());
                }
            }
